...
class LaneTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # NOTE: previously this dataset cropped the transformed image to the last
        # `crop_size` rows with `img = img[:,-self.crop_size:,:]`. For datasets
        # where the full frame (no top cropping) is required (e.g. your
        # 1640x590 images) we keep the entire transformed image here so the
        # image returned matches the transform (which should already resize
        # image -> mask consistently). If you still want to crop, set
        # `crop_size` handling here accordingly.
...
class LaneClsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Ensure image and label/mask keep the same size after any simulated
        # geometric transform. This will catch mis-alignments early.
        assert img.size == label.size, 'Image and label size mismatch after simu_transform: %s vs %s' % (str(img.size), str(label.size))
