...
class LaneExternalIterator(object):
    def __init__(self, path, list_path, batch_size=None, shard_id=None, num_shards=None, mode = 'train', dataset_name=None, anno_cache=None):
        ...
        if self.mode == 'train':
            ...
            elif dataset_name == 'CULane_cropped':
                cache_path = os.path.join(path, 'culane_anno_cache.json')
            else:
                raise NotImplementedError
...
def ExternalSourceTrainPipeline(batch_size, num_threads, device_id, external_data, train_width, train_height, top_crop, normalize_image_scale = False, nscale_w = None, nscale_h = None, augment=True):
    ...
    # top of the image which was done previously with `top_crop`.
    ...
    images = fn.crop_mirror_normalize(images,
                     ...
                     # crop equals resize -> no cropping occurs
                     crop=(train_height, train_width), crop_pos_x=0., crop_pos_y=0.)
...
def ExternalSourceValPipeline(batch_size, num_threads, device_id, external_data, train_width, train_height):
    ...
    with pipe:
        ...
        images = fn.decoders.image(jpegs, device="mixed")
        # Resize to final evaluation resolution without top crop
        images = fn.resize(images, resize_x=train_width, resize_y=train_height)
        images = fn.crop_mirror_normalize(images,
                                         dtype=types.FLOAT,
                                         mean=[0.485 * 255, 0.456 * 255, 0.406 * 255],
